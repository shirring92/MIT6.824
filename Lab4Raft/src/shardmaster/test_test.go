...
func check(t *testing.T, groups []int, ck *Clerk) {
	...
	log.Printf("---------------------check: shards allocate ok")
	...
	log.Printf("---------------------check: shards balance ok")
}
...
func TestBasic(t *testing.T) {
	...
	check(t, []int{gid2}, ck)
	cfa[4] = ck.Query(-1)
	log.Printf("---------------------cfg 4 %v", cfa[4])
	ck.Leave([]int{gid2})
	cfa[5] = ck.Query(-1)
	log.Printf("---------------------cfg 5 %v", cfa[5])
	fmt.Printf("  ... Passed\n")
	...
	for s := 0; s < nservers; s++ {
		...
		log.Printf("---------------------shutdown server %d", s)
		for i := 0; i < len(cfa); i++ {
			c := ck.Query(cfa[i].Num)
			log.Printf("---------------------query %d: c %v", cfa[i].Num, c)
	...
	fmt.Printf("Test: Move ...\n")
	{
		...
		for i := 0; i < NShards; i++ {
			cf := ck.Query(-1)
			log.Printf("---------------------query cf %v", cf)
			if i < NShards/2 {
				ck.Move(i, gid3)
				log.Printf("---------------------move i %d gid3 %d", i, gid3)
				if cf.Shards[i] != gid3 {
					cf1 := ck.Query(-1)
					log.Printf("---------------------query cf1 %v", cf1)
					if cf1.Num <= cf.Num {
				...
				ck.Move(i, gid4)
				log.Printf("---------------------move i %d gid4 %d", i, gid4)
				if cf.Shards[i] != gid4 {
					cf1 := ck.Query(-1)
					log.Printf("---------------------query cf1 %v", cf1)
					if cf1.Num <= cf.Num {
