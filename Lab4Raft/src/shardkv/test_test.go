...
func TestStaticShards(t *testing.T) {
	...
	va := make([]string, n)
	for i := 0; i < n; i++ {
		log.Printf("----------------------iterate %d", i)
		ka[i] = strconv.Itoa(i) // ensure multiple shards
		va[i] = randstring(20)
		ck.Put(ka[i], va[i])
		log.Printf("----------------------put <%v, %v>", ka[i], va[i])
	}
	for i := 0; i < n; i++ {
		check(t, ck, ka[i], va[i])
		log.Printf("----------------------check <%v, %v>", ka[i], va[i])
	}
	...
	for xi := 0; xi < n; xi++ {
		...
		go func(i int) {
			...
			log.Printf("----------------------check <%v, %v> after shutdown", ka[i], va[i])
		}(xi)
	}
	...
	for i := 0; i < n; i++ {
		check(t, ck, ka[i], va[i])
		log.Printf("----------------------check <%v, %v> after restart", ka[i], va[i])
	}
...
func TestJoinLeave(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		...
		ck.Append(ka[i], x)
		va[i] += x
		log.Printf("----------------------append %v to <%v, %v> after join 1", x, ka[i], va[i])
	}
	...
	for i := 0; i < n; i++ {
		...
		ck.Append(ka[i], x)
		va[i] += x
		log.Printf("----------------------append %v to <%v, %v> after leave 0", x, ka[i], va[i])
	}
...
func TestSnapshot(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		...
		va[i] = randstring(20)
		ck.Put(ka[i], va[i])
		log.Printf("----------------------put <%v, %v> for (0)", ka[i], va[i])
	}
	for i := 0; i < n; i++ {
		check(t, ck, ka[i], va[i])
		log.Printf("----------------------check <%v, %v> for (0)", ka[i], va[i])
	}

	cfg.join(1)
	cfg.join(2)
	cfg.leave(0)
	log.Printf("----------------------join 1, 2, leave 0")

	for i := 0; i < n; i++ {
		check(t, ck, ka[i], va[i])
		log.Printf("----------------------check <%v, %v> for (1, 2)", ka[i], va[i])
		x := randstring(20)
		ck.Append(ka[i], x)
		va[i] += x
		log.Printf("----------------------append %v to <%v, %v> for (1, 2)", x, ka[i], va[i])
	}
	...
	for i := 0; i < n; i++ {
		check(t, ck, ka[i], va[i])
		log.Printf("----------------------check <%v, %v> for (0, 2)", ka[i], va[i])
		x := randstring(20)
		ck.Append(ka[i], x)
		va[i] += x
		log.Printf("----------------------append %v to <%v, %v> for (0, 2)", x, ka[i], va[i])
	}
	...
	cfg.ShutdownGroup(1)
	cfg.ShutdownGroup(2)
	log.Printf("----------------------shutdown 0, 1, 2")

	cfg.StartGroup(0)
	cfg.StartGroup(1)
	cfg.StartGroup(2)
	log.Printf("----------------------restart 0, 1, 2")

	for i := 0; i < n; i++ {
		check(t, ck, ka[i], va[i])
		log.Printf("----------------------check <%v, %v> for (0, 1, 2) after restart", ka[i], va[i])
	}
...
func TestMissChange(t *testing.T) {
	...
	log.Printf("----------------------join 0 start")
	cfg.join(0)
	log.Printf("----------------------join 0 finish")
	...
	for i := 0; i < n; i++ {
		ka[i] = strconv.Itoa(i) // ensure multiple shards
		va[i] = randstring(20)
		log.Printf("----------------------put <%v, %v> for (0) start", ka[i], va[i])
		ck.Put(ka[i], va[i])
		// log.Printf("----------------------put <%v, %v> for (0) finish", ka[i], va[i])
	}
	for i := 0; i < n; i++ {
		log.Printf("----------------------check <%v, %v> for (0) start", ka[i], va[i])
		check(t, ck, ka[i], va[i])
		// log.Printf("----------------------check <%v, %v> for (0) finish", ka[i], va[i])
	}

	log.Printf("----------------------join 1 start")
	cfg.join(1)
	log.Printf("----------------------join 1 finish")

	log.Printf("----------------------shutdown server (0, 0) start")
	cfg.ShutdownServer(0, 0)
	log.Printf("----------------------shutdown server (1, 0) start")
	cfg.ShutdownServer(1, 0)
	log.Printf("----------------------shutdown server (2, 0) start")
	cfg.ShutdownServer(2, 0)
	log.Printf("----------------------shutdown server (0, 0), (1, 0), (2, 0) finish")

	log.Printf("----------------------join 2 start")
	cfg.join(2)
	log.Printf("----------------------leave 1 start")
	cfg.leave(1)
	log.Printf("----------------------leave 0 start")
	cfg.leave(0)
	log.Printf("----------------------join 2, leave 0, 1 finish")

	for i := 0; i < n; i++ {
		log.Printf("----------------------check <%v, %v> for (2) start", ka[i], va[i])
		check(t, ck, ka[i], va[i])
		x := randstring(20)
		log.Printf("----------------------append %v to <%v, %v> for (2) start", x, ka[i], va[i])
		ck.Append(ka[i], x)
		va[i] += x
		log.Printf("----------------------append %v to <%v, %v> for (2) finish", x, ka[i], va[i])
	}
	...
	for i := 0; i < n; i++ {
		log.Printf("----------------------check <%v, %v> for (1, 2) start", ka[i], va[i])
		check(t, ck, ka[i], va[i])
		x := randstring(20)
		log.Printf("----------------------append %v to <%v, %v> for (1, 2) start", x, ka[i], va[i])
		ck.Append(ka[i], x)
		va[i] += x
		log.Printf("----------------------append %v to <%v, %v> for (1, 2) finish", x, ka[i], va[i])
	}

	log.Printf("----------------------restart (0, 0) start")
	cfg.StartServer(0, 0)
	log.Printf("----------------------restart (1, 0) start")
	cfg.StartServer(1, 0)
	log.Printf("----------------------restart (2, 0) start")
	cfg.StartServer(2, 0)
	log.Printf("----------------------restart (0, 0), (1, 0), (2, 0) finish")
	...
	log.Printf("----------------------shutdown (0, 1) start")
	cfg.ShutdownServer(0, 1)
	log.Printf("----------------------shutdown (1, 1) start")
	cfg.ShutdownServer(1, 1)
	log.Printf("----------------------shutdown (2, 1) start")
	cfg.ShutdownServer(2, 1)
	log.Printf("----------------------shutdown (0, 1), (1, 1), (2, 1) finish")

	log.Printf("----------------------join 0 start")
	cfg.join(0)
	log.Printf("----------------------leave 2 start")
	cfg.leave(2)
	log.Printf("----------------------join 0, leave 2 finish")

	for i := 0; i < n; i++ {
		log.Printf("----------------------check <%v, %v> for (0, 1) start", ka[i], va[i])
		check(t, ck, ka[i], va[i])
		x := randstring(20)
		log.Printf("----------------------append %v to <%v, %v> for (0, 1) start", x, ka[i], va[i])
		ck.Append(ka[i], x)
		va[i] += x
		log.Printf("----------------------append %v to <%v, %v> for (0, 1) finish", x, ka[i], va[i])
	}

	log.Printf("----------------------restart (0, 1) start")
	cfg.StartServer(0, 1)
	log.Printf("----------------------restart (1, 1) start")
	cfg.StartServer(1, 1)
	log.Printf("----------------------restart (2, 1) start")
	cfg.StartServer(2, 1)
	log.Printf("----------------------restart (0, 1), (1, 1), (2, 1) finish")
...
func TestConcurrent1(t *testing.T) {
	...
	ff := func(i int) {
		...
		for atomic.LoadInt32(&done) == 0 {
			x := randstring(5)
			log.Printf("----------------------append %v to <%v, %v> for (0) start", x, ka[i], va[i])
			ck1.Append(ka[i], x)
			va[i] += x
			log.Printf("----------------------append %v to <%v, %v> for (0) finish", x, ka[i], va[i])
			time.Sleep(10 * time.Millisecond)
		}
		log.Printf("----------------------append %v finish", i)
	}
	...
	log.Printf("----------------------leave 0 start")
	cfg.leave(0)
	log.Printf("----------------------join 1, 2, leave 0 finish")

	log.Printf("----------------------shutdown 0 start")
	cfg.ShutdownGroup(0)
	time.Sleep(100 * time.Millisecond)
	log.Printf("----------------------shutdown 1 start")
	cfg.ShutdownGroup(1)
	time.Sleep(100 * time.Millisecond)
	log.Printf("----------------------shutdown 2 start")
	cfg.ShutdownGroup(2)
	log.Printf("----------------------shutdown 0, 1, 2 finish")

	log.Printf("----------------------leave 2 start")
	cfg.leave(2)
	log.Printf("----------------------leave 2 finish")

	time.Sleep(100 * time.Millisecond)
	log.Printf("----------------------restart 0 start")
	cfg.StartGroup(0)
	log.Printf("----------------------restart 1 start")
	cfg.StartGroup(1)
	log.Printf("----------------------restart 2 start")
	cfg.StartGroup(2)
	log.Printf("----------------------restart 0, 1, 2 finish")
	...
	log.Printf("----------------------join 1 start")
	cfg.join(1)
	log.Printf("----------------------join 0, leave 1, join 1 finish")
	...
	for i := 0; i < n; i++ {
		<-ch
		log.Printf("----------------------%v done", ka[i])
	}
	

	for i := 0; i < n; i++ {
		check(t, ck, ka[i], va[i])
		log.Printf("----------------------check <%v, %v> for (0, 1)", ka[i], va[i])
	}
...
func TestConcurrent2(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		...
		va[i] = randstring(1)
		ck.Put(ka[i], va[i])
		log.Printf("----------------------put <%v, %v> for (0,1,2)", ka[i], va[i])
	}
	...
	ff := func(i int, ck1 *Clerk) {
		...
		for atomic.LoadInt32(&done) == 0 {
			...
			ck1.Append(ka[i], x)
			va[i] += x
			log.Printf("----------------------append %v to <%v, %v> for (0,1,2)", x, ka[i], va[i])
	...
	cfg.leave(0)
	cfg.leave(2)
	log.Printf("----------------------leave 0, 2")
	time.Sleep(3000 * time.Millisecond)
	cfg.join(0)
	cfg.join(2)
	cfg.leave(1)
	log.Printf("----------------------join 0, 2, leave 1")
	time.Sleep(3000 * time.Millisecond)
	cfg.join(1)
	cfg.leave(0)
	cfg.leave(2)
	log.Printf("----------------------join 1, leave 0, 2")
	time.Sleep(3000 * time.Millisecond)

	cfg.ShutdownGroup(1)
	cfg.ShutdownGroup(2)
	log.Printf("----------------------shutdown 1, 2")
	time.Sleep(1000 * time.Millisecond)
	cfg.StartGroup(1)
	cfg.StartGroup(2)
	log.Printf("----------------------restart 1, 2")
	...
	for i := 0; i < n; i++ {
		check(t, ck, ka[i], va[i])
		log.Printf("----------------------check <%v, %v> for (1)", ka[i], va[i])
	}
