...
func GenericTest(t *testing.T, part string, nclients int, unreliable bool, crash bool, partitions bool, maxraftstate int) {
	...
	for i := 0; i < 3; i++ {
		...
		go spawn_clients_and_wait(t, cfg, nclients, func(cli int, myck *Clerk, t *testing.T) {
			// log.Printf("-----------------------------------------start spawn_clients_and_wait on %d", cli)
			j := 0
			defer func() {
				clnts[cli] <- j
			}()
			...
			// log.Printf("-----------------------------------------%d: client try Put key %v", cli, key)
			Put(cfg, myck, key, last)
			// log.Printf("-----------------------------------------%d: client finish put key %v", cli, key)
			for atomic.LoadInt32(&done_clients) == 0 {
				if (rand.Int() % 1000) < 500 {
					...
					// log.Printf("-----------------------------------------%d: client got Append val %v", cli, last)
					j++
				} else {
					// log.Printf("-----------------------------------------%d: client new Get key %v\n", cli, key)
					v := Get(cfg, myck, key)
					// log.Printf("-----------------------------------------%d: client got Get val %v", cli, v)
					if v != last {
						log.Fatalf("-----------------------------------------%d: client get wrong value, key %v, wanted:\n%v\n, got\n%v\n", cli, key, last, v)
					}
				}
			}
			// log.Printf("-----------------------------------------%d: client done, quit now", cli)
		})
		...
		if partitions {
			...
			// has started.
			// log.Printf("-----------------------------------------connect all in partitions\n")
			cfg.ConnectAll()
...
func TestSnapshotSize3B(t *testing.T) {
	...
	for i := 0; i < 200; i++ {
		...
		// t5 := time.Now()
		// tc2 := t5.Sub(t4)
		// log.Printf("--------------------------------------------check <x, 1>")
	}

	// check that servers have thrown away most of their log entries
	sz := cfg.LogSize()
	// log.Printf("--------------------------------------------check log size %v", sz)
	if sz > 8*maxraftstate {
		t.Fatalf("logs were not trimmed (%v > 8*%v)", sz, maxraftstate)
	}
	...
	ssz := cfg.SnapshotSize()
	// log.Printf("--------------------------------------------check snapshot size %v", ssz)
