...
func (rf *Raft) AppendEntries(args *AppendEntriesArgs, reply *AppendEntriesReply) {
	...
	if args.Term >= rf.currentTerm {
		...
		// 	go rf.breakElection(rf.currentTerm) // lose election, back to follower
		// }
	...
	if args.PrevLogIndex < len(rf.log) + rf.logOffset {
		...
		// }
		// if rf.state == Candidate {
		// 	go rf.breakElection(rf.currentTerm) // lose election, back to candidate
		// }
		// rf.mu.Unlock()
		rf.persist()
	...
	// if args.PrevLogIndex < rf.logOffset || len(rf.log) + rf.logOffset <= args.PrevLogIndex || rf.log[args.PrevLogIndex - rf.logOffset].Term != args.PrevLogTerm { // log inconsistency
	// 	if args.PrevLogIndex < rf.logOffset { // append log entries already applied to raft
	// 		reply.PrevIndex = rf.logOffset
	// 		reply.PrevTerm = rf.log[0].Term
	// 	} else if len(rf.log) + rf.logOffset <= args.PrevLogIndex { // leader has logs that missing in follower's log entries
	// 		reply.PrevIndex = len(rf.log) - 1 + rf.logOffset
	// 		reply.PrevTerm = rf.log[reply.PrevIndex - rf.logOffset].Term
	// 	} else if rf.log[args.PrevLogIndex - rf.logOffset].Term != args.PrevLogTerm { // last applied log term is not identical for leader and follower
	// 		i := args.PrevLogIndex - rf.logOffset
	// 		t := rf.log[i].Term
	// 		for i > 0 {
	// 			if rf.log[i].Term != t {
	...
	// 			i--
	// 		}
	// 		reply.PrevIndex = i + rf.logOffset
	...
	// 	var j = args.PrevLogIndex + 1 - rf.logOffset
...
func (rf *Raft) sendAppendEntries(server int) bool {
	rf.mu.Lock()
	// if rf.nextIndex[server] <= rf.logOffset {
	// 	rf.mu.Unlock()
	// 	return false
	// }
	...
	// if reply.Term == rf.currentTerm { //reply false, reply.Term <= rf.currentTerm, rf.nextIndex[server] > 0
	// 	if reply.PrevIndex < rf.logOffset {
	// 		rf.mu.Unlock()
	// 		s, lastindex, lastterm := rf.readSnapshot()
	// 		rf.mu.Lock()
	// 		DPrintf(" %d send InstallSnapshot to %d on AE reply", rf.me, server)
	// 		go rf.sendInstallSnapshot(server, lastindex, lastterm, s)
	// 		return false
	// 	}
	// 	if rf.nextIndex[server] >= reply.PrevIndex + 1 {
	// 		rf.nextIndex[server] = reply.PrevIndex + 1
	// 	} else if rf.nextIndex[server] > 1 + rf.logOffset {
	// 		rf.nextIndex[server]--
	// 	} else {
	// 		rf.nextIndex[server] = 1 + rf.logOffset
	// 	}
	// 	DPrintf(" %d reduce %d nextIndex to %d on AE reply", rf.me, server, rf.nextIndex[server])
	// 	return false
	// }

	if reply.ConflicTerm > 0 && reply.ConflicTerm <= rf.log[len(rf.log) - 1].Term{
		// var i = previdx
		// for i > rf.logOffset - 1 {
		// 	if reply.ConflicTerm >= rf.log[i - rf.logOffset].Term {
		// 		break
		// 	}
		// 	i--
		// }
		// for i < len(rf.log) + rf.logOffset {
		// 	t := 0
		// 	if i >= rf.logOffset {
		// 		t = rf.log[i - rf.logOffset].Term
		// 	}
		// 	if reply.ConflicTerm < t {
		// 		break
		// 	}
		// 	i++
		// }
		// if i == len(rf.log) + rf.logOffset {
		// 	rf.nextIndex[server] = reply.ConflicIndex // if conflict term not exist in leader's log
		// } else {
		// 	rf.nextIndex[server] = i // next index is conflict term last log's next index
		// }
...
func (rf *Raft) sendInstallSnapshot(server int, i int, t int, s Snapshot) bool {
	...
	DPrintf(" %d sendInstallSnapshot %v to %d", rf.me, s, server)
...
func (rf *Raft) AEProcess() {
	...
	for {
		...
		if t == 0 {
			...
			DPrintf(" %d in AEProcess %d on term %d", rf.me, next - 1, rf.currentTerm)
			for i := 0; i < len(rf.peers); i++ {
				if i == rf.me {
					continue
				}
...
func (rf *Raft) updateCommitIndex(index int) {
	// var start int 
	// // var end int
	// DPrintf(" %d start updateCommitIndex %d, lastApplied %d", rf.me, rf.commitIndex, rf.lastApplied)
	// for {
	// 	if rf.killed() {
	// 		return
	// 	}
	...
	// 	}
		...
		// end = len(rf.log) - 1 + rf.logOffset
		// DPrintf(" %d start updateCommitIndex, start %d, end %d, matchindex %d", rf.me, start, end, rf.matchIndex[rf.me])
		// var mid int
		cnt := 0
		// for start <= end {
		...
		if cnt >= len(rf.peers) / 2 + 1 && rf.log[index - rf.logOffset].Term == rf.currentTerm {
			...
			DPrintf(" %d update commitIndex to %d, lastapplied is %d", rf.me, rf.commitIndex, rf.lastApplied)
		}

		// DPrintf("%d finish updateCommitIndex, start %d, end %d, log length %d, offset %d", rf.me, start, end, len(rf.log), rf.logOffset)
		// if end < len(rf.log) + rf.logOffset && end > rf.commitIndex && rf.log[end - rf.logOffset].Term == rf.currentTerm {
		// 	rf.commitIndex = end
		// 	DPrintf(" %d update commitIndex to %d, lastapplied is %d", rf.me, end, rf.lastApplied)
		// }
		// rf.mu.Unlock()
...
func (rf *Raft) applyCommitedMsg() {
	...
	for {
		...
		if rf.commitIndex > rf.lastApplied {
			DPrintf(" %d start apply commited msg %d, lastApplied is %d", rf.me, rf.commitIndex, rf.lastApplied)
			rf.mu.Unlock()
